...
const VendorUpdate1 = ({vendor, handleUpdateVendor}) => {
    ...
    return(
        <li
            className={'w-[450px] h-[610px] bg-[#21243A] flex flex-col  pt-[25px] rounded-[15px] font-primary'}
            key={vendor.id}
        >
            ...
            <div className={'px-[20px] mt-[30px]'}>
                <div className={'flex flex-col '}>
                    <div className={'flex flex-col gap-[5px] '}>
                        ...
                        <div className={'flex justify-between '}>
                            <input name={'code'} onChange={updateDataFunc} disabled={isActive} className={'w-[358px] h-[60px] px-[18px] py-[10px] bg-transparent text-black outline-0 text-[18px] bg-[#5C5F7E] rounded-[15px] text-[18px]'} defaultValue={vendorData.code}/>
                ...
                <div className={'flex flex-col mt-[20px]'}>
                    <div className={'flex flex-col gap-[5px]  '}>
                        ...
                        <div className={'flex justify-between '}>
                            <input name={'webhook'} onChange={updateDataFunc} disabled={isActive} className={'w-[358px] h-[60px] px-[18px] py-[10px] bg-transparent text-black outline-0 text-[18px] bg-[#5C5F7E] rounded-[15px] text-[18px]'} defaultValue={vendorData.webhook}/>
                            <img src={edit} className={'self-center'} />
                        </div>
                    </div>
                </div>
                <div className={'flex flex-col mt-[20px]'}>
                    <div className={'flex flex-col gap-[5px]  '}>
                        ...
                        <div className={'flex justify-between '}>
                            <input name={'legalEntity'} onChange={updateDataFunc} disabled={isActive} className={'w-[358px] h-[60px] px-[18px] py-[10px] bg-transparent text-black outline-0 text-[18px] bg-[#5C5F7E] rounded-[15px] text-[18px]'} defaultValue={vendorData.legalEntity}/>
                            <img src={edit} className={'self-center'} />
                        </div>
                    </div>
                </div>
                <div className={'flex justify-between gap-[20px] mt-[40px]'}>
                    <button onClick={handleBtn} className={'py-[18px] px-[25px] w-[196px] h-[60px] text-white text-[18px] bg-[#F62E46] rounded-[15px]'}>{isActive ? 'Редактивровать' : 'Обновить'}</button>
                    <button className={'flex items-center justify-center w-[196px] h-[60px] py-[18px] px-[25px] gap-[10px] text-white text-[18px] bg-transparent rounded-[15px] border'}>
                        <img src={postaIco}/>
                        <span>Постаматы</span>
                    </button>
                </div>
